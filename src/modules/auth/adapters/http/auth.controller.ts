...
export class AuthController {
  ...
  @Post('login') async login(@Body() dto: LoginDto, @Res({ passthrough: true }) response: Response) { const result = await this.service.login(dto); this.cookie(response, result.refreshToken); return { accessToken: result.accessToken }; }
