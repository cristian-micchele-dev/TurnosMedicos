...
export interface TokenService { signAccess(payload:Record<string,unknown>):string; signRefresh(payload:Record<string,unknown>):string; verifyAccess(token:string):Record<string,unknown>; verifyRefresh(token:string):Record<string,unknown>; refreshTtlMs?():number }
