...
export class AuthService {
  ...
  async login(dto: LoginDto) { const user = await this.users.findByEmail(dto.email.trim().toLowerCase()); if (!user || !user.active || !(await this.hasher.verify(user.passwordHash, dto.password))) throw new UnauthorizedError(); return this.issue(user, undefined, Boolean(dto.rememberMe)); }
